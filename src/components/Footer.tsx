...
export function SocialLinks({ id = "social-links", className = "" }: SocialLinksProps) {
  return (
    <div className={`flex space-x-4 ${className}`} id={id}>
      <SocialLink 
        platform="facebook" 
        href="#" 
        id={`${id}-facebook`}
...
export function ContactInfo({ className = "", id = "footer-contact-section" }: ContactInfoProps) {
  return (
    <motion.div variants={itemVariants} className={className} id={id}>
      <h4 className="text-lg font-semibold mb-4" id={`${id}-title`}>Contact</h4>
      <ul className="space-y-3 text-gray-400" id={`${id}-list`}>
        <li id={`${id}-email`}>support@wiztechrecovery.com</li>
        <li id={`${id}-phone`}>+1 (555) 123-4567</li>
        <li id={`${id}-address`}>123 Recovery Lane</li>
        <li id={`${id}-city`}>Tech City, TC 12345</li>
      </ul>
    </motion.div>
  );
}
...
export function BusinessHours({ className = "", id = "footer-hours-section" }: BusinessHoursProps) {
  return (
    <motion.div variants={itemVariants} className={className} id={id}>
      <h4 className="text-lg font-semibold mb-4" id={`${id}-title`}>Hours</h4>
      <ul className="space-y-3 text-gray-400" id={`${id}-list`}>
        <li id={`${id}-weekday`}>Monday - Friday: 9AM - 6PM</li>
        <li id={`${id}-saturday`}>Saturday: 10AM - 4PM</li>
        <li id={`${id}-sunday`}>Sunday: Closed</li>
        <li id={`${id}-emergency`}>24/7 Emergency Service Available</li>
      </ul>
    </motion.div>
  );
}
...
export function QuickLinks({ 
  ...
  links = [
    { text: "About Us", to: "/about", id: "footer-link-about" },
    { text: "Services", to: "/services", id: "footer-link-services" },
    { text: "Blog", to: "/blog", id: "footer-link-blog" },
    { text: "Support", to: "/support", id: "footer-link-support" }
  ]
}: QuickLinksProps) {
  return (
    <motion.div variants={itemVariants} className={className} id={id}>
      <h4 className="text-lg font-semibold mb-4" id={`${id}-title`}>Quick Links</h4>
      <ul className="space-y-3 text-gray-400" id={`${id}-list`}>
        {links.map((link) => (
          <li key={link.to} id={link.id}>
...
export function Copyright({ className = "", id = "footer-copyright" }: CopyrightProps) {
  return (
    <motion.div
      variants={itemVariants}
...
export function FooterBackground({ className = "", id = "footer-background" }: FooterBackgroundProps) {
  return (
    <div className={`absolute inset-0 ${className}`} id={id}>
      <div 
...
export function Footer({ className = "", id = "main-footer" }: FooterProps) {
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4" id={`${id}-container`}>
        <motion.div
          ...
          id={`${id}-content`}
        >
          <div className="grid grid-cols-1 md:grid-cols-12 gap-12" id={`${id}-grid`}>
            <CompanyInfo />

            <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-8" id={`${id}-info-columns`}>
              <ContactInfo />
              <BusinessHours />
              <QuickLinks />
            </div>
